 ...
 * 所以上线前务必在高德控制台把本站域名加进该 key 的白名单。
 *
 * 安全密钥（jscode）**不在这个文件里，也不在前端任何地方**：
 * 它以环境变量注入 nginx 容器，由服务端在转发 _AMapService 请求时追加。
 * 高德文档对明文写进前端的原话是「不建议在生产环境使用（不安全）」。
 * 见 ops/nginx.conf.template。
 */
export const AMAP = {
   ...
   * 但纯静态托管（GitHub Pages 无反向代理）没有服务端可藏密钥，这是唯一可行的路。
   * 靠**域名白名单**兜底：在高德控制台给上面这个 key 绑定允许的域名，
